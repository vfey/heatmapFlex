#' @title Tools to generate zoomable and splittable heatmaps
#' @docType package
#' @name heatmapS
...
#' @param distfun (\code{function}). Distance function. Defaults to \code{dist}.
#' @param hclustfun (\code{function}). Hierarchical clutering function. Defaults to \code{hclust}.
...
#' @param col (\code{character}) Name (acronym) of the colour palette to use. Can be one of "RdBkGn" (c("green", "black", "red")), "BuYl" (c("blue", "yellow")), "BuWtRd" (c("blue", "white", "red")) or a valid name used by \code{RColorBrewer::brewer.pal}. Defaults to NULL which will set "RdBkGn".
...
#' @param sidebars (\code{factor} or \code{numeric}). Categorical variable or numeric matrix describing rows or columns of the heatmap.
#' @param picketdata (\code{numeric}). Numeric matrix for picket plot.
#' @param r.cex (\code{numeric}). Font size for row labels.
#' @param c.cex (\code{numeric}). Font size for column labels.
#' @param titleheight (\code{numeric} or \code{lcm(numeric)}). Relative or absolute height (using \code{lcm}, see \code{layout}) of the plot title.
#' @param dendroheight (\code{numeric} or \code{lcm(numeric)}). Relative or absolute height (using \code{lcm}, see \code{layout}) of the dendrogram.
#' @param dendrowidth (\code{numeric} or \code{lcm(numeric)}). Relative or absolute width (using \code{lcm}, see \code{layout}) of the dendrogram.
...
#' @param picketheight (\code{numeric} or \code{lcm(numeric)}). Relative or absolute height (using \code{lcm}, see \code{layout}) of the picket plots.
#' @param sidebarwidth (\code{numeric} or \code{lcm(numeric)}). Relative or absolute width (using \code{lcm}, see \code{layout}) of the sidebars.
...
#' @param plot (\code{logical}). Draw the plot?
#' @param factorpalettefn (\code{character}). Name of the colour palette.
#' @param add.sig (\code{logical}). Should significance asterixes be drawn?
#' @param pv (\code{numeric}). Matrix of the same dimensions as \code{x} with P-Values for correlation significance.
#' @param order_list (\code{logical}). Should the order of the correlation matrix be reversed? Meaningful if the order of input variables should be preserved because \code{image} turns the input matrix.
#' @param genes2highl (\code{character}). Vector of gene symbols (or whatever labels are used) to be highlighted. If not \code{NULL} will draw a semi-transparent rectangle around the labels and rows or columns in the heatmap labels.
#'
#' @details The result can be used for zooming. For simple basic cases, picketvar can be given directly. Factor sidebars are supported, but legends are only shown for the first two.
...
#' \tab distfun (\code{function}). Distance function. Defaults to \code{dist}\cr
#' \tab hclustfun (\code{function}). Hierarchical clutering function. Defaults to \code{hclust}\cr
...
#' \tab col (\code{character}) Name (acronym) of the colour palette to use. Can be one of "RdBkGn" (c("green", "black", "red")), "BuYl" (c("blue", "yellow")), "BuWtRd" (c("blue", "white", "red")) or a valid name used by \code{RColorBrewer::brewer.pal}. Defaults to NULL which will set "RdBkGn"\cr
